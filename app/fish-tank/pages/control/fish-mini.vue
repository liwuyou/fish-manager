  ...
  <view class="control-page">
    <view class="status-header" :class="{ offline: !status.online }">
      <view class="status-row">
        ...
        <text class="model-info-header">{{ deviceModel }}</text>
      </view>
    </view>

    <!-- 传感器数据 -->
    <view class="sensor-section">
      <view class="sensor-grid two-col">
        ...
        <view class="sensor-item">
          ...
          <text class="sensor-label">水质</text>
          <text class="sensor-value">{{ parseInt(status.tdsValue) }}ppm <text class="wq-tag" :class="waterQualityClass">{{ waterQualityText }}</text></text>
        </view>
      </view>
    </view>

    <!-- 大图标按钮 -->
    <view class="big-btn-section">
      <view class="big-btn-row">
        <view class="big-btn-wrapper">
          <view
            class="big-btn"
            :class="status.systemPowered ? 'power-on' : 'power-off'"
            @click="togglePower"
          >
            <text class="big-btn-icon">⚡</text>
            <text class="big-btn-label">{{ status.systemPowered ? '开机' : '关机' }}</text>
          </view>
        </view>
        <view class="big-btn-wrapper">
          <view
            class="big-btn feed-btn"
            :class="{ busy: status.servoMoving }"
            @click="triggerServo"
          >
            <text class="big-btn-icon">🐟</text>
            <text class="big-btn-label">{{ status.servoMoving ? '运行中' : '喂食' }}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="control-section">
      ...
      <view class="control-card">
        <view class="control-header">
          <text class="control-name" @click="startRename('light', '灯光')">{{ getCtrlName('light', '灯光') }}</text>
...
.wq-excellent { background: #e8f5e9; color: #2e7d32; }
.wq-good { background: #e3f2fd; color: #1565c0; }
.wq-fair { background: #fff3e0; color: #e65100; }
.wq-poor { background: #ffebee; color: #c62828; }
...
.big-btn {
  ...
  &.power-on {
    ...
    .big-btn-label { color: #2e7d32; }
  }

  &.power-off {
    background: linear-gradient(135deg, #ffebee, #ffcdd2);
    .big-btn-label { color: #c62828; }
  }

  &.feed-btn {
    background: linear-gradient(135deg, #e3f2fd, #bbdefb);
    .big-btn-label { color: #1565c0; }

    &.busy {
      background: linear-gradient(135deg, #fff3e0, #ffe0b2);
      .big-btn-label { color: #e65100; }
    }
  }
}
