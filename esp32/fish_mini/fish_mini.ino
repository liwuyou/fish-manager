...
// 作者: 李无忧
// 描述: WiFi + WebSocket远程控制版本 (fish_mini 精简版)

#define MODEL_NAME "fish_mini"

// ==================== 引脚定义 ====================
#define RELAY_PIN     25   // 继电器1 - 控制12V+5V电源
#define RELAY2_PIN    27   // 继电器2 - GPIO27
#define RELAY3_PIN    19   // 继电器3(备用开关2) - GPIO19
#define PWM1_PIN      18   // 水泵1 - GPIO18, LEDC通道4
#define PWM3_PIN      23   // 灯条 - GPIO23, LEDC通道6
#define PWM_AIR_PUMP_PIN  17   // 气泵 - GPIO17, LEDC通道2
...
// ==================== 远程服务器配置 ====================
#define WS_SERVER_HOST "112.124.107.159"
...
void handleRoot() {
  ...
  html += "<div class='info'><b>型号:</b><br><span class='code'>" + String(MODEL_NAME) + "</span></div>";
...
void handleStatus() {
  ...
  snprintf(buf, sizeof(buf), 
    "{\"pwm1Level\":%d,\"pwm3Level\":%d,\"airPumpLevel\":%d,\"servoMoving\":%s,\"adcWQVoltage\":%.2f,\"adcTempVoltage\":%.2f,\"waterTemperature\":%.1f,\"tdsValue\":%.0f,\"systemPowered\":%s}",
    pwm1Level, pwm3Level, airPumpLevel,
...
void wsSendStatus() {
  ...
  status["relay3State"] = relay3State;
...
void wsSendCmdResult(String cmdId, String cmd, bool success, String message) {
  ...
  result["relay3State"] = relay3State;
...
void updateOLED() {
  ...
  char line1[32];
  snprintf(line1, sizeof(line1), "M1:%d LED:%d", pwm1Level, pwm3Level);
  u8g2.drawStr(0, 10, line1);
  
  char line2[32];
  snprintf(line2, sizeof(line2), "Air:%d", airPumpLevel);
...
void loop() {
  ...
  if (millis() - lastDebug > 10000) {
    ...
    DPRINT(" Air:"); DPRINT(airPumpLevel);
